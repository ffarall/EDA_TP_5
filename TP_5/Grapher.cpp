...
void Grapher::update(Worm * wArray[], int wormCount)
{
	...
	{
		al_draw_bitmap(get_bitmap(wArray[i]->get_currentState(), wArray[i]->get_frameCounter()), wArray[i]->get_pos().get_x(), wArray[i]->get_pos().get_y(), (wArray[i]->get_orientation()) ? ALLEGRO_FLIP_HORIZONTAL : NO_ROTATION);
	}
	al_flip_display();
...
ALLEGRO_BITMAP * Grapher::get_bitmap(wormState_n currentState, uint frameCounter)
{
	switch (currentState)
	{
		...
		case MOVING:
		case STOP_MOVING:
			if ((frameCounter >= 1) && (frameCounter <= 8) || (frameCounter >= 1+14) && (frameCounter <= 8+14) || (frameCounter >= 1 + 14*2) && (frameCounter <= 8 + 14*2))
			{
				return walkingImages[frameCounter + 3 - 1];
			}
			else if ((frameCounter >= 9) && (frameCounter <= 13) || (frameCounter >= 9+14) && (frameCounter <= 13+14 || (frameCounter >= 9+14*2) && (frameCounter <= 13+14*2)))
			{
				return walkingImages[frameCounter + 2 - 1];
			}
